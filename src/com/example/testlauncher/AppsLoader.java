...
public class AppsLoader extends AsyncTaskLoader<ArrayList<AppModel>> {
    ...
    public AppsLoader(final Context context) {
        super(context);
    ...
    public ArrayList<AppModel> loadInBackground() {
        ...
        final ArrayList<AppModel> items = new ArrayList<AppModel>(apps.size());
        for (int i = 0; i < apps.size(); i++) {
            final String pkg = apps.get(i).packageName;

            // only apps which are launchable
            if (context.getPackageManager().getLaunchIntentForPackage(pkg) != null) {
                final AppModel app = new AppModel(context, apps.get(i));
    ...
    @Override
    public void deliverResult(final ArrayList<AppModel> apps) {
        if (isReset()) {
        ...
        final ArrayList<AppModel> oldApps = apps;
        mInstalledApps = apps;
    ...
    @Override
    public void onCanceled(final ArrayList<AppModel> apps) {
     ...
     * @param apps
     */
    protected void onReleaseResources(final ArrayList<AppModel> apps) {
        // do nothing
    }
    ...
    public static final Comparator<AppModel> ALPHA_COMPARATOR = new Comparator<AppModel>() {
        ...
        @Override
        public int compare(final AppModel object1, final AppModel object2) {
